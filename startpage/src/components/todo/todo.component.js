class Todo extends Component {
  ...
  addFilter({ target }, filter) {
    const hasFilterEnabled =
      this.refs.taskList.getAttribute("filter") === filter;

    this.refs.filters.forEach((filter) => filter.classList.remove("active"));

    if (!hasFilterEnabled) target.classList.add("active");
  ...
  handleTaskActionsVisibility() {
    ...
    else this.refs.cleanTasks.classList.remove("active");
  }
  ...
  moveToDirection(task, direction, animateOnly = false) {
    ...
    setTimeout(() => {
      ...
      const currentIndex = Math.max(
        0,
        directions.isUp(direction) ? previousIndex - 1 : previousIndex + 1,
      );
      ...
      if (!animateOnly)
        if (directions.isUp(direction))
          this.refs.taskList.insertBefore(
            task,
            task?.previousElementSibling || task,
          );
        else this.refs.taskList.insertBefore(task.nextElementSibling, task);
  ...
  setHeightOffset(task, direction) {
    const applyOffset = (element) => {
      ...
      element?.setAttribute(
        "style",
        `--offset-height: ${offset - offset * 2}px`,
      );
    };

    applyOffset(
      Tasks.directions.isUp(direction)
  ...
  move({ id, attributes }) {
    ...
    else this.moveToDirection(task.nextElementSibling, directions.UP, true);
  ...
  cleanTasks() {
    if (!confirm(`Clean all your tasks?`)) return;
  ...
  setTaskObserver() {
    ...
    const taskObserver = new MutationObserver((mutations) => {
      mutations.forEach((mut) => {
        if (mut.attributeName === "move-direction") this.move(mut.target);
