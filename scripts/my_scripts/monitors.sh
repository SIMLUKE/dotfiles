...
ROFI_THEME="$XDG_CONFIG_HOME/rofi/style-MONITORS.rasi"
ROFI_PROMPT="Monitor"
MONITOR_PROFILES_DIR="$XDG_DATA_HOME/hypr/monitor-profiles"
...
save_layout() {
    ...
    local safe_name
    safe_name=$(echo "$layout_name" | tr ' ' '_' | tr -cd '[:alnum:]_-')
    local layout_file="$MONITOR_PROFILES_DIR/${safe_name}.conf"

    mkdir -p "$MONITOR_PROFILES_DIR"
    
    # Create new layout file with timestamp
    {
        echo "# Monitor Layout: $layout_name"
        echo "# Created: $(date)"
        echo "# Generated by monitors.sh"
        echo ""
        hyprctl monitors -j | jq -r '.[] | "monitor = \(.name),\(.width)x\(.height)@\(.refreshRate),\(.x)x\(.y),1"' 2>/dev/null ||
            hyprctl monitors | grep -E "^Monitor" | sed 's/^/monitor = /'
    } > "$layout_file"

    notify-send "Monitor Manager" "Layout '$layout_name' saved to $layout_file"
}

load_layout() {
    [[ ! -d "$MONITOR_PROFILES_DIR" ]] && {
    ...
    local layout_files
    layout_files=$(find "$MONITOR_PROFILES_DIR" -name "*.conf" -type f 2>/dev/null)
    [[ -z "$layout_files" ]] && {
        notify-send "Monitor Manager" "No layouts available"
        return
    }

    # Extract layout names from files
    local layouts=""
    while IFS= read -r file; do
        local name
        name=$(grep "^# Monitor Layout:" "$file" | sed 's/^# Monitor Layout: //')
        [[ -n "$name" ]] && layouts="$layouts$name|$(basename "$file")
"
    done <<<"$layout_files"

    # Show layout selection menu
    local selected
    selected=$(echo "$layouts" | cut -d'|' -f1 | rofi -dmenu -i -p "Load Layout" -theme "$ROFI_THEME")
    [[ -z "$selected" ]] && return

    # Find the corresponding file
    local selected_file
    selected_file=$(echo "$layouts" | grep "^$selected|" | cut -d'|' -f2)
    [[ -z "$selected_file" ]] && return

    # Apply the layout
    local layout_path="$MONITOR_PROFILES_DIR/$selected_file"
    ...
    notify-send "Monitor Manager" "Layout '$selected' applied"
}
